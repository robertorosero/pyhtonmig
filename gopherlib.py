...
import logging
_log = logging.getLogger('py.gopherlib')
...
def get_directory(f):
    ...
    while 1:
        line = f.readline()
        if not line:
            _.log.info ('Unexpected EOF from server)')
            break
        if line[-2:] == CRLF:
            line = line[:-2]
        elif line[-1:] in CRLF:
        ...
        if line == '.':
            break
        if not line:
            _.log.info ('(Empty line from server)')
            continue
        gtype = line[0]
        parts = line[1:].split(TAB)
        if len(parts) < 4:
            _.log.info ('(Bad line from server: %r)' % (line,))
            continue
        if len(parts) > 4:
            if parts[4:] != ['+']:
                _.log.info ('(Extra info from server:'),
                _.log.info (parts[4:], ')')
        else:
            parts.append('')
