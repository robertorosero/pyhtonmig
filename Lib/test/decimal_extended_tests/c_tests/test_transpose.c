...
/*
 * All the following is pasted from transpose.c and transpose3.c for the sole
 * reason of running the tests with uint8_t arrays, which allow greater sizes.
 */
 ...
 */
static void
_block_transpose_3xpow2_c(uint8_t *matrix, mpd_size_t rows, mpd_size_t cols)
{
	uint8_t buf1[BMEMB];
	uint8_t buf2[BMEMB];
 ...
 */
void
transpose_3xpow2_c(uint8_t *matrix, mpd_size_t rows, mpd_size_t cols)
{
	uint8_t buf[3*BMEMB];
	uint8_t *p;
	...
	if (msize <= bsize) {
		std_trans_c(buf, matrix, rows, cols);
		...
		_block_transpose_3xpow2_c(matrix, rows, cols);
		for (p = matrix; p < matrix+msize; p += bsize) {
			std_trans_c(buf, p, rows, BMEMB);
		...
		for (p = matrix; p < matrix+msize; p += bsize) {
			std_trans_c(buf, p, BMEMB, cols);
			memcpy(p, buf, bsize * (sizeof *matrix));
		}
		_block_transpose_3xpow2_c(matrix, rows, cols);
	}
	else {
		mpd_err_fatal("transpose_3xpow2_c: wrong input for rows or cols");
	}
}
...
#define UMOD_ARRAY3 (3*(1ULL<<26))
#define UCHAR_ARRAY (1ULL<<29)
#define UCHAR_ARRAY3 (3*(1ULL<<29))
...
testit_uint(void (* func)(mpd_uint_t *, mpd_size_t, mpd_size_t), mpd_size_t rows, mpd_size_t cols)
{
	...
	fprintf(stderr, "size: %10"PRI_mpd_size_t"\tstd_trans: %f sec\t "
                        "in_place_trans: %f sec\n",
		msize,
...
int main(void)
{
	...
	for (rows = 1; mul_size_t(rows, rows) <= UMOD_ARRAY; rows *= 2) {
		testit_uint(transfunc_uint transpose_pow2, rows, rows);
	}

	fprintf(stderr, "\n2^n * 2*2^n mpd_uint_t matrices:\n\n");
	for (rows = 8, cols = 16; mul_size_t(rows, cols) <= UMOD_ARRAY; rows *= 2, cols*=2) {
		testit_uint(transfunc_uint transpose_pow2, rows, cols);
	}
	...
	for (rows = 1; mul_size_t(rows, rows) <= UCHAR_ARRAY; rows *= 2) {
		testit_uchar(transfunc_uchar transpose_pow2_c, rows, rows);
	}

	fprintf(stderr, "\n2^n * 2*2^n uint8_t matrices:\n\n");
	for (rows = 8, cols = 16; mul_size_t(rows, cols) <= UCHAR_ARRAY; rows *= 2, cols*=2) {
		testit_uchar(transfunc_uchar transpose_pow2_c, rows, cols);
	}
	...
	fprintf(stderr, "\n3 * 2^n mpd_uint_t matrices:\n\n");
	for (rows = 3, cols = 4; mul_size_t(rows, cols) <= UMOD_ARRAY3; cols*=2) {
		testit_uint(transpose_3xpow2, rows, cols);
	}

	fprintf(stderr, "\n2^n * 3 mpd_uint_t matrices:\n\n");
	for (rows = 4, cols = 3; mul_size_t(rows, cols) <= UMOD_ARRAY3; rows *= 2) {
		testit_uint(transpose_3xpow2, rows, cols);
	}


	fprintf(stderr, "\n3 * 2^n uint8_t matrices:\n\n");
	for (rows = 3, cols = 4; mul_size_t(rows, cols) <= UCHAR_ARRAY3; cols*=2) {
		testit_uchar(transpose_3xpow2_c, rows, cols);
	}

	fprintf(stderr, "\n2^n * 3 uint8_t matrices:\n\n");
	for (rows = 4, cols = 3; mul_size_t(rows, cols) <= UCHAR_ARRAY3; rows *= 2) {
		testit_uchar(transpose_3xpow2_c, rows, cols);
	}
