"""Temporary unit tests for BytesIO"""
...
class TestGenericBytesIO(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self._fp = self.MODULE.BytesIO(self._lines)
...
class TestBytesIO(TestGenericBytesIO):
    MODULE = _bytes_io
    ...
    def test_unicode(self):
        ...
        # The cBytesIO module converts Unicode strings to character
        # strings when writing them to cBytesIO objects.
        ...
        f = self.MODULE.BytesIO(unicode(self._line[:5]))
        s = f.getvalue()
        self.assertEqual(s, 'abcde')
        self.assertEqual(type(s), types.StringType)

        self.assertRaises(UnicodeEncodeError, self.MODULE.BytesIO,
                          unicode('\xf4', 'latin-1'))
...
class TestBufferBytesIO(TestBytesIO):
    constructor = buffer
