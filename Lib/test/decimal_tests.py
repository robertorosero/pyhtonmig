...
if HAVE_CDECIMAL and (not HAVE_CONFIG_64):
    ...
    def read_unlimited(v, context):
        savetraps = context._traps
        saveflags = context._flags
        context._traps = 0
        vv = Decimal(v, context)
        context._traps = savetraps
        if context._flags & DecInvalidOperation:
            # Do it the hard way
            context._flags = saveflags
            vv = readcontext.create_decimal(v)
        return vv
else:
    def read_unlimited(v, context):
        return Decimal(v, context)
...
cdecimal_status_diff = {}
cdecimal_result_diff = {}
cdecimal_skipped_disagreements = []
if HAVE_CDECIMAL:
    if HAVE_CONFIG_64:
        # 64 bit version: With a reduced working precision in mpd_qpow()
        # the status matches.
        cdecimal_status_diff = {
            "pwsx803": ([Inexact, Rounded, Subnormal, Underflow],
                        [Inexact, Rounded]),
            "pwsx805": ([Inexact, Rounded, Subnormal, Underflow],
                        [Inexact, Rounded])
            }
    # These are skipped for decNumber, too.
    cdecimal_result_diff = {
    ...
    cdecimal_skipped_disagreements = ['pwmx325', 'pwmx326']
# END CDECIMAL SKIPS
...
class DecimalTest(unittest.TestCase):
    ...
    def eval_equation(self, s):
        ...
        if id in skipped_test_ids or id in cdecimal_skipped_disagreements: # XXX
            return
        ...
        if id in cdecimal_status_diff: # XXX
            t = cdecimal_status_diff[id]
            theirexceptions = t[0]
        ...
        for i, val in enumerate(valstemp):
            ...
            if fname in ('to_sci_string', 'to_eng_string'):
                ...
                v = read_unlimited(v, self.context) # XXX
            vals.append(v)
    ...
    def change_precision(self, prec):
        if (not HAVE_CDECIMAL) or HAVE_CONFIG_64:
...
class DecimalUsabilityTest(unittest.TestCase):
    ...
    def test_immutability_operations(self):
        ...
        # XXX Cdecimal: checked extensively in the library runtest.
