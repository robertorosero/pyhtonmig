%TODO: dynload #! restriction is too strong
...
\date{24 Januari 1994 \\ Release 1.0.0 BETA} % XXX update before release!
...
a Python program executes \code{import foo}, the search for modules
tries to find a file \file{foomodule.o} or \file{foomodule.so} (in the
...
\code{ftp.cwi.nl}, directory \file{pub/dynload}.  The files are
\file{dl-1.6.tar.Z} (SGI IRIX 4 dynamic loading),
\code{dld-3.2.3.tar.Z} (GNU dld) and \file{dl-dld-1.1.tar.Z} (SGI dl
on top of GNU dld).
