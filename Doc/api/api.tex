...
installed in \file{\envvar{prefix}/include}.
...
true if (and only if) the object pointed to by \var{a} is a Python list.
...
\cfunction{Py_IsInitialized()} returns true if Python is currently in the
...
\cdata{PyExc_Exception}).  In this case the
\member{__module__} attribute of the new class is set to the first
part (up to the last dot) of the \var{name} argument, and the class
name is set to the last part (after the last dot).  When the user has
specified the \code{-X} command line option to use string exceptions,
for backward compatibility, or when the \var{base} argument is not a
class object (and not \NULL{}), a string object created from the
entire \var{name} argument is returned.  The \var{base} argument can
be used to specify an alternate base class.  The \var{dict} argument
...
string objects, depending on the use of the \code{-X} option to the
...
loaded.)  Return a new reference to the imported module, or
...
If the objects pointed to by \code{*\var{p1}} and
...
%begin{latexonly}
\strong{Note:} this function returns a ``borrowed'' reference.
%end{latexonly}
\end{cfuncdesc}
...
success and \code{-1} on failure (in which case an exception will be
set).
\end{cfuncdesc}
...
\begin{cfuncdesc}{int}{PyDict_SetItemString}{PyDictObject *p,
            char *key,
            PyObject *val}
Inserts \var{value} into the dictionary using \var{key} as a
key. \var{key} should be a \ctype{char *}.  The key object is created
using \code{PyString_FromString(\var{key})}.
\end{cfuncdesc}
...
\var{pylong}.  If \var{pylong} is greater than \constant{LONG_MAX}, an 
\exception{OverflowError} is raised.
\end{cfuncdesc}

\begin{cfuncdesc}{unsigned long}{PyLong_AsUnsignedLong}{PyObject *pylong}
Returns a C \ctype{unsigned long} representation of the contents of 
\var{pylong}.  If \var{pylong} is greater than \constant{ULONG_MAX},
an \exception{OverflowError} is raised.
\end{cfuncdesc}
...
flag for \var{flags} is \constant{Py_PRINT_RAW}; if given, the
...
module object.  This function never fails.  \strong{Note:} this
...
\cfunction{main()} function with the difference that the first entry
should refer to the script file to be executed rather than the
executable hosting the Python interpreter.  If there isn't a script
that will be run, the first entry in \var{argv} can be an empty
string.  If this function fails to initialize \code{sys.argv}, a fatal 
condition is signalled using \cfunction{Py_FatalError()}.
...
control on it, even if she manipulates regularly object pointers to
...
library: \cfunction{malloc()}, \cfunction{calloc()},
\cfunction{realloc()} and \cfunction{free()}. This will result in
...
\begin{cfuncdesc}{TYPE*}{PyMem_NEW}{TYPE, size_t n}
Same as \cfunction{PyMem_Malloc()}, but allocates \var{(n *
sizeof(TYPE))} bytes of memory. Returns a pointer casted to
\ctype{TYPE*}.
\end{cfuncdesc}

\begin{cfuncdesc}{TYPE*}{PyMem_RESIZE}{ANY *p, TYPE, size_t n}
Same as \cfunction{PyMem_Realloc()}, but the memory block is resized
to \var{(n * sizeof(TYPE))} bytes. Returns a pointer casted to
\ctype{TYPE*}.
\end{cfuncdesc}
...
Here is the example from section 1, rewritten so that the I/O buffer is
allocated from the Python heap by using the first function set:
