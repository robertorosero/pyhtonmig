...
standard input), \code{sys.argv[0]} is set to \code{'-'}.  When the
\code{-c command} form is used, \code{sys.argv[0]} is set to
\code{'-c'}.  Options found after \code{-c command} are not consumed
...
\item
When the Python interpreter is invoked with the \code{-O} flag (that's
the letter, not the number), optimized code is generated and stored in
\file{.pyo} files.  The optimizer currently doesn't help much; it only
removes \keyword{assert} statements and \code{SET_LINENO} instructions.
When \code{-O} is used, \emph{all} bytecode is optimized;
\file{.pyc} files are ignored and \file{.py} files are compiled to
optimized bytecode.
