...
import socket
import logging
_log=logging.getLogger('py.nntplib')
...
class NNTP:
    ...
    def getwelcome(self):
        ...
        if self.debugging: _log.info('*welcome*'), repr(self.welcome)
        return self.welcome
    ...
    def putline(self, line):
        ...
        line = line + CRLF
        if self.debugging > 1: _log.info('*put*'), repr(line)
        self.sock.sendall(line)

    def putcmd(self, line):
        """Internal: send one command to the server (through putline())."""
        if self.debugging: _log.info('*cmd*'), repr(line)
        self.putline(line)
